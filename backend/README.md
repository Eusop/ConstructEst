...
- **No live distance/geolocation.** The frontend computes store distances via straight-line (haversine) distance from a fixed Tarlac City reference point, not the user's real location — wiring in real driving distance would need a routing API (e.g. OSRM, which like the map tiles has a free public instance) and a small frontend change.
